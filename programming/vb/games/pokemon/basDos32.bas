...
Public Function FindSendWindow() As Long
    ...
    FindSendWindow& = 0&
End Function
Public Function FindSendWindowA() As Long
    Dim AOL As Long, MDI As Long, child As Long
    Dim SendStatic As Long
    AOL& = FindWindow("AOL Frame25", vbNullString)
    MDI& = FindWindowEx(AOL&, 0&, "MDIClient", vbNullString)
...
Public Sub MailToListSent(TheList As ListBox)
    ...
    Next AddMails&
End Sub
Public Sub SendMailA(Person As String, subject As String, message As String)
...
Public Function RoomCount() As Long
    ...
    RoomCount& = Count&
End Function
Public Sub AddRoomToListbox(TheList, AddUser As Boolean)
...
Public Sub WaitForOKOrRoom(Room As String)
    ...
    DoEvents
End Sub
Public Sub MemberRoom(ky, Room As String)
    Call Keyword("aol://2719:" & ky & "-2-" & Room$)
End Sub
Public Sub PublicRoom(Room As String)
    Call Keyword("aol://2719:21-2-" & Room$)
End Sub
Public Function PrivateRoom(Room As String)
...
Public Function IMLastMsg() As String
    ...
    Dim NewSpot As Long
    If Not FindIM <> 0 Then Exit Function
    Rich& = FindWindowEx(FindIM&, 0&, "RICHCNTL", vbNullString)
...
Public Function FileExists(sFileName As String) As Boolean
    ...
    End If
End Function
Sub LoadTextA(txtLoad, PathA As String)
...
Public Sub Pausea(Duration As Long)
    Dim Current As Long
    Current = Timer
...
Public Sub RunMenuByString(SearchString As String)
    ...
    Next LookFor&
End Sub
Function servermail(SN$)
    Do
        DoEvents
        ClickForward
    Loop Until FindSendWindow <> 0
    MailForward SN$, "<B>Team Water Mail Server", True
    ClickKeepAsNew
End Function
Function clearchat()
    Pausea 0.7
    ChatSend ("<font color=#fffffe>blah<font color=""#fefefe""><pre" & String(1900, "  "))
    ChatSend ("<font color=#fffffe>blah<font color=""#fefefe""><pre" & String(1900, "   "))
    ChatSend ("<font color=#fffffe>blah<font color=""#fefefe""><pre" & String(1900, "     "))
End Function
Public Sub PartSend(Chat As String)
    Dim Room As Long, AORich As Long, AORich2 As Long
    Room& = FindRoom&
    AORich& = FindWindowEx(Room, 0&, "RICHCNTL", vbNullString)
    AORich2& = FindWindowEx(Room, AORich, "RICHCNTL", vbNullString)
...
Sub AddtoAOL(frm As Form, xpos, ypos)
    ...
    sett% = SetParent(frm.hWnd, waol%)
    sett% = ShowWindow(waol%, 2)
    sett% = ShowWindow(waol%, 3)
End Sub
Public Sub Locate(scrn As String)
    Keyword "aol://3548:" & scrn
End Sub
Sub KillGlyph()
    Dim AOL%, aotool%, aotool2%, glyph%
    AOL% = FindWindow("AOL Frame25", vbNullString)
...
Sub KillTwo()
    ...
    ToolIcon& = FindWindowEx(Toolbar&, ToolIcon&, "_AOL_Icon", vbNullString)
    ToolIcon& = FindWindowEx(Toolbar&, ToolIcon&, "_AOL_Icon", vbNullString)
    Call SendMessage(ToolIcon&, WM_CLOSE, 0, 0)
End Sub
Sub ClickKeepAsNew()
    Dim aoicon%, l
    aoicon% = FindWindowEx(FindMailBox, 0&, "_AOL_Icon", vbNullString)
    For l = 1 To 2
        aoicon% = FindWindowEx(FindMailBox, aoicon%, "_AOL_Icon", vbNullString)
    Next l
...
Sub ClickForward()
    Dim aoicon%, l
    aoicon% = FindWindowEx(FindForwardWindow, 0&, "_AOL_Icon", vbNullString)
    For l = 1 To 6
        aoicon% = FindWindowEx(FindForwardWindow, aoicon%, "_AOL_Icon", vbNullString)
    Next l
    Call PostMessage(aoicon%, WM_LBUTTONDOWN, 0&, 0&)
    Call PostMessage(aoicon%, WM_LBUTTONUP, 0&, 0&)
End Sub
Function ClickSignOn()
    ...
    wchild% = FindWindowEx(wmdi%, 0&, "AOL Child", vbNullString)
    wicon& = FindWindowEx(wchild%, 0&, "_AOL_Icon", vbNullString)
    wicon& = FindWindowEx(wchild%, wicon&, "_AOL_Icon", vbNullString)
    wicon& = FindWindowEx(wchild%, wicon&, "_AOL_Icon", vbNullString)
    wicon& = FindWindowEx(wchild%, wicon&, "_AOL_Icon", vbNullString)
    Icon wicon&
End Function
...
EliteTalker = Made$
End Function
Function StringInList(str As String, lst) As Boolean
    num$ = 0
    numa$ = 0
syt:
    If num$ = lst.ListCount Then
        StringInList = False
        Exit Function
    Else
        If InStr(TrimSpaces(LCase(lst.List(num$))), TrimSpaces(LCase(str))) <> 0 Then
            StringInList = True
            Exit Function
        End If
        num$ = num$ + 1
    ...
    End If
End Function
Function FindIndexInList(str As String, lst)
    num$ = 0
    numa$ = 0
syt:
    If num$ = lst.ListCount Then
    ...
    End If
End Function
Function SendListToChat(lst As ListBox, delay As Long)
    num$ = 0
asd:
    If num$ = lst.ListCount Then
        Exit Function
