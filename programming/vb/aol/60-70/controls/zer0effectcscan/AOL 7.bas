...
Public Function ShorterText1$(ByVal hwnd&, ByVal theline&)
    Dim findchar&
    Dim TheChar$
    Dim TheChars$
    Dim TempNum&
    ...
    For findchar& = 1 To TextLength&
        ...
        Let TheChars$ = TheChars$ & TheChar$
            ...
            End If
    Next
        Let ShorterText1$ = TheChars$
    Exit Function
skipit:
    Let TheText$ = ReplaceText$(TheText$, Chr(13), "")
    Let ShorterText1$ = TheText$
End Function
...
End Sub
Sub SendMail(who, subject, Message)
...
SetText whoto&, who
SetText Subh&, subject
SetText Messj&, Message
Hand1& = FindWindowEx(OurParent&, 0, "_AOL_Icon", vbNullString)
Hand2& = FindWindowEx(OurParent&, Hand1&, "_AOL_Icon", vbNullString)
...
End Sub
Sub ClickMail(AOLIcon As String)
sendnow = SendMessageLong(AOLIcon, WM_LBUTTONDOWN, 0, 0)
sendnow = SendMessageLong(AOLIcon, WM_LBUTTONUP, VK_SPACE, 0)
End Sub
...
Dim AOLFrame As Long, MDIClient As Long, AOLChild As Long, Caption
AOLFrame = FindWindow("aol frame25", vbNullString)
MDIClient = FindWindowEx(AOLFrame, 0&, "mdiclient", vbNullString)
AOLChild = FindWindowEx(MDIClient, 0&, "aol child", vbNullString)
RICHCNTL = FindWindowEx(AOLChild, 0&, "richcntl", vbNullString)
...
If InStr(Caption, ">") = 1 Then
Exit Sub
End If
If InStr(Caption, "IM") >= 1 Then
Exit Sub
End If
If InStr(Caption, "Write Mail") >= 1 Then
Exit Sub
End If
...
ChatBox = (RICHCNTL&)
End Function
Public Function chatroom() As String
Dim RICHCNTL As Long, textlen As Long, RICHCNTLTxt As String
Dim AOLFrame As Long, MDIClient As Long, AOLChild As Long, txt1 As String
AOLFrame = FindWindow("aol frame25", vbNullString)
MDIClient = FindWindowEx(AOLFrame, 0&, "mdiclient", vbNullString)
...
End Function
Public Sub Ims_On()
Call SendIM("$IM_ON", "Bye")
End Sub
Public Sub Ims_Off()
Call SendIM("$IM_OFF", "Bye")
End Sub
...
txt1 = ReplaceString(txt1, Mid(txt1, InStr(txt1, "Link -1 of "), InStr(txt1, "Link -1 of ") + 2), "")
skip:
If txt1 = "" Then GoTo Start:
GetLastChatLine = txt1
End Function
Function GetLastSN(chatline As String)
Dim Txt
Txt = Left(chatline, InStr(chatline, ":") - 1)
GetLastSN = Txt
End Function
Function GetLastMSG(chatline As String)
Dim txt1 As String, space, Txt
...
Loop Until Count2& = Count1& And Count3& = Count1&
End Sub
...
End Sub
Public Sub EnterPR(ver As String, Room As String)
If ver = 0 Then
Call Keyword7("aol://2719:2-2-" & Room)
ElseIf ver = 1 Then
Call Keyword7("aol://2719:10-4-" & Room)
Else
MsgBox ("not a valid version, 0 for aol 6 and below, 1 for aol 7 or higher")
End If
End Sub
