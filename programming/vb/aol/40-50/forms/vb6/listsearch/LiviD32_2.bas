...
'Call IMKeyword("" + List1.List(X) + "", "Sup " + List1.List(X) + "? " + Text1 + Chr(10) + Chr(13) + Chr(10) + Chr(13) + " " & Text2.Text & "")
'Next X
...
Public Sub MakeMailLists(lst1 As ListBox, lst2 As ListBox)
...
Y = lst1.ListCount
Loop Until X = Y
Call MailListFlash(lst1, True)
For z = 0 To lst1.ListCount - 1
lst2.AddItem (lst1.List(z))
If lst2.ListCount = 500 Then Call MailSend(AOLUser(), "-^v^--• silver mail list", ListToString(lst1))
Next z
Call MailSend(AOLUser(), "-^v^--• silver mail list", ListToString(lst1))
End Sub
Function ListToString(TheList As ListBox)
Dim dolist As Long
For dolist = 0 To TheList.ListCount - 1
ListToString = ListToString & TheList.List(dolist) + Chr(10) + Chr(13) + ""
Next dolist
If TheList.ListCount = 0 Then Exit Function
ListToString = Mid(ListToString, 1, Len(ListToString) - 2)
End Function
Function ServerFind(whotosendto As String, whattofind As String, lst1 As ListBox, lst2 As ListBox)
lst1.Clear
lst2.Clear

Dim z As Long
For z = 0 To lst1.ListCount - 1
If InStr(lst1.List(z), whattofind) Then lst2.AddItem (lst1.List(z))
Next z
Call MailSend(whotosendto, "-^v^--• silver finds for " & whattofind, "here")
