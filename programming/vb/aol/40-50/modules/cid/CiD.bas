...
' This Bas was made by CiD
' For some codes check out www.nwozone.com/knk4o
' Thanks for testing this .bas.
' There will be a v2 later on!
' -CiD-
...
End Type
Function add(x As TextBox, z As TextBox)
Exponent = x + z
End Function
...
Function RGBtoRGB_Fade(A1 As Label, A2 As Label, A3 As Label, B1 As Label, B2 As Label, B3 As Label, Text1)
    a = Len(Text1)
    For B = 1 To a
        c = Left(Text1, B)
        D = Right(c, 1)
...
Function SendSpi(txt As TextBox)
B = Len(txt)
For a = 1 To B
c = Right((txt), 1)
Text_StripLetter (txt), 1
e = c + txt
SendChat (e)
Pause 0.5
Next a
...
Sub AOLTitle(NewTitle As String)
Call AOLSetText(AOLWindow(), NewTitle)
End Sub
Sub DestroyIM()
...
Sub File_Delete(sFileName As String)
...
'Example: text9.text = "Your system's CPU type is: " & GetCPUType
...
End Sub
Function Vizline(Linx As Line)
Linx.Visible = True
End Function
Function NoVizline(Linx As Line)
Linx.Visible = False
End Function
Sub WaitFormModalOK()
Do
...
Function Frame(frmForm As Form)
frmForm.Line (0, intScaleHeight)-(0, 0), 0
       frmForm.Line (0, 0)-(intScaleWidth - 1, 0), 0
       frmForm.Line (intScaleWidth - 1, 0)-(intScaleWidth - 1, intScaleHeight - 1), 0
       frmForm.Line (0, intScaleHeight - 1)-(intScaleWidth - 1, intScaleHeight - 1), 0
...
AOL% = FindWindow("AOL Frame25", vbNullString)
toolbar% = FindChildByClass(AOL%, "AOL Toolbar")
hde% = ShowWindow(toolbar%, SW_SHOW)
End Sub
Sub AOLHideWelcome()
Dim x
...
TxtLen = Len(txt)
before = Right$(txt, which - 1)
beforelen = Len(before)
afterthat = TxtLen - beforelen - 1
After = Left$(txt, afterthat)
...
Sub AOLIMOff()
Call AOLInstantMessage("$IM_OFF", "Pointless?")
End Sub

Sub AOLIMsOn()
Call AOLInstantMessage("$IM_ON", "Pointless?")
