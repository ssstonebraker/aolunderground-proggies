...
'player and volume control.  To control
'volume correctly though, every control in
'the volume program must be checked or
'else you'll try turning up the CD volume
'and it'll turn up like the MIDI volume.
...
'Closes edit play list
    Dim win As Long, Win2 As Long, Btn As Long
    win& = FindWindow("SJE_CdPlayerClass", vbNullString)
    Win2& = FindWindowEx(win&, 0&, "#32770", vbNullString)
    Btn& = FindWindowEx(Win2&, 0&, "Button", vbNullString)
    Btn& = FindWindowEx(Win2&, Btn&, "Button", vbNullString)
    Btn& = FindWindowEx(Win2&, Btn&, "Button", vbNullString)
    Btn& = FindWindowEx(Win2&, Btn&, "Button", vbNullString)
    Btn& = FindWindowEx(Win2&, Btn&, "Button", vbNullString)
    Btn& = FindWindowEx(Win2&, Btn&, "Button", vbNullString)
